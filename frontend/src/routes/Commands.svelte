<script lang="ts">
  ...
  const currentCommand = $derived(
    selectedMode === "download" ? downloadCommand : qualityCommand,
  );
  ...
  <div class="box flex flex-col w-full">
    ...
    <div class="p-2 flex flex-col gap-2">
      <input
        ...
        readonly
        class="text-xl box p-2 w-full cursor-text focus:!bg-bg-muted"
      />
      <button class="w-full box p-2" onclick={copyCommand}
        >{copyFeedback ? "Copied!" : "Copy to clipboard"}</button
      >
    </div>
  </div>
  <div class="flex flex-col gap-4">
    ...
    >
      ...
      <div class="p-2 flex flex-col items-center gap-2">
        ...
        <input
          ...
          placeholder="Artist/album/playlist/song link(s) // Separate with space"
        ...
        <label class="flex w-full items-center justify-between cursor-pointer">
          ...
          <input
            type="checkbox"
            class="sr-only peer"
            bind:checked={downloadOverwrite}
          />
          <div
        ...
        <div class="flex w-full items-center justify-between gap-2">
          ...
          <div class="flex flex-wrap justify-end">
            {#each codecs as opt}
              <label
                class="box px-2 py-1 text-sm cursor-pointer hover:bg-bg-active! {downloadCodec ===
                opt.value
                  ? 'bg-bg-active!'
                  : ''}"
              >
        ...
        <label class="flex w-full items-center justify-between cursor-pointer">
          ...
          <input
            type="checkbox"
            class="sr-only peer"
            bind:checked={downloadIncludeParticipate}
          />
          <div
            class="w-5 h-5 box flex items-center justify-center text-text text-sm leading-none peer-checked:after:content-['✕'] after:content-['']"
          ></div>
    ...
    >
      ...
      >
        ...
        />
        <span
          >Quality <span class="underline cursor-help"
            ><Popup
              long
              text="Checks which qualities are available for the given link."
              position="left">[?]</Popup
            ></span
          ></span
        >
      ...
      <div class="p-2 flex flex-col items-center gap-2">
        <input
          type="text"
          class="box p-2 text-sm w-full"
          placeholder="Song/album/playlist URL"
          bind:value={qualityUrl}
        />
      </div>
    </div>
  </div>
