  ...
  <div class="box flex flex-col w-full">
    ...
    <div class="grid grid-cols-[1fr_auto]">
      <div class="flex flex-col gap-1 p-2">
        ...
        <div class="flex mt-auto justify-between *:text-sm *:text-text-muted">
          <span
            >made with &lt;3 by <a
              onclick={() => BrowserOpenURL("https://gs0.me/")}
              href="#">@gabors0</a
            >
            under the <b>MIT License</b></span
          >
          <a
            onclick={() =>
              BrowserOpenURL("https://github.com/gabors0/AMDecrypt-gui")}
            href="#">github</a
          >
        </div>
      </div>
      <div
        class="border-l border-border aspect-square w-32 h-32 flex items-center justify-center"
      >
    ...
    <div class="p-2 text-center text-sm text-text-muted">
      ...
      <a
        ...
        href="#">wrapper-manager</a
      >
      by
      <a
        onclick={() => BrowserOpenURL("https://github.com/WorldObservationLog")}
        href="#">@WorldObservationLog</a
      >
    ...
    <div class="p-2 text-center text-sm text-text-muted">
      <div class="flex w-full items-center justify-between gap-2">
        ...
        <div class="flex flex-wrap justify-end">
          {#each themes as opt}
            <label
              class="box border-r-0 px-2 py-1 text-sm cursor-pointer hover:bg-bg-active! {selectedTheme ===
              opt.value
                ? 'bg-bg-active!'
                : ''}"
            >
    ...
    <div class="p-2 text-sm text-text-muted">
      ...
      <span
        ondblclick={() => (devTestVisible = !devTestVisible)}
        class="list-none italic">v{version}</span
      >
      {#if devTestVisible}
        <div class="flex flex-col gap-2">
          ...
          <button
            class="box p-2"
            onclick={() => appendLog("[ERROR] this is just a test :)")}
            >test error</button
          >
        </div>
      {/if}
